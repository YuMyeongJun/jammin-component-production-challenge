...
describe('<Collapse />', () => {
  ...
  it('렌더링 체크', () => {
    ...
    expect(collapse?.className.includes(collapseClasses.header.root)).toBeTruthy();
  });
  ...
  it('제목 노출 여부 체크', () => {
    ...
    const title = screen
      .getAllByRole('button')
      .find((btn) => btn.className === 'bc-collapse-header')?.firstChild
      ?.firstChild as HTMLSpanElement;

    // expect(title.classList.contains(collapseClasses.header.label.root)).toBeTruthy();
    expect(title.className.includes(collapseClasses.header.label.content)).toBeTruthy();
  ...
  it('collapse 버튼 노출 여부', () => {
    ...
    // 버튼 노출
    const showIcon = screen.getAllByRole('button')[0].lastChild as HTMLButtonElement;
