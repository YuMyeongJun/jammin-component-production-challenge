...
export const Upload = forwardRef<HTMLInputElement, IUploadProps>(
  (args, ref) => {
    ...
    const {
      ...
      previewClassName = "jammin-upload-preview",
      isError,
        ...
        for (const i of e.target.files) {
          if (
            ...
            !SUPPORTED_FORMATS.some(
              (arrFormat) =>
                i.type?.split("/")[0] === arrFormat.replace("/*", ""),
            )
          ) {
            e.target.files = null;
            e.target.value = "";
    ...
    return (
      <>
        <label
          ...
          onDragOver={
            args.shape === "drag" && !useCustomOnDrag
              ? handleDragOver
              : undefined
          }
          onDragLeave={
          ...
          onDrop={
            args.shape === "drag" && !useCustomOnDrag ? handleDrop : undefined
          }
        >
          ...
          <div className={previewClassName}>
              ...
              ? files?.map((file, i) => (
                  ...
                  >
                    ...
                    <ButtonEx
                      type="text"
                      onClick={() =>
                        handleDeleteFile(file.name, file.lastModified, i)
                      }
                    >
            ...
            {usePreview
              ? imageUrl?.map((image, i) => (
                  <img src={image} alt="uploadImage" key={i} />
                ))
              : null}
          </div>
        ) : null}
